...
void ballMovement(struct Ball *ball, float deltaTime, int windowWidth,
  ...
  if (ball->x + ball->radius >= windowWidth) {
    ...
    ball->velocityX = std_random(300, 100);
    ball->velocityY = std_random(300, 100);
...
reset:
  ...
  while (WindowShouldClose() == false) {
    ...
    // If ball is paused do not update the logic;
    if (isPaused == false) {
      // Ball movement
    ...
    if (playerTwo.score >= 10) {
      ...
      DrawText("Player Two wins", MeasureText("Player Two wins", 70),
               (windowHeight / 2), 70, WHITE);
    }
    if (playerOne.score >= 10 || playerTwo.score >= 10) {

      DrawText("Press R to restart", 300, 0, 70, GOLD);
      isPaused = true;
